...
本次改动（在不改变原有功能/口径前提下）：
1) 报告中增加“银行余额总额”（跨银行汇总的总余额）；
2) 输出目录结构调整（你说的“xxx”=人名）：
   输出根目录固定为：工作目录/输出
   - 汇总文件（所有人-合并交易流水.xlsx、所有人-重复交易流水.xlsx 等）仍放在：工作目录/输出（保持不变）
   - 每个人单独一个文件夹（即“xxx”为人名）：
        工作目录/输出/<人名>/银行/...
        工作目录/输出/<人名>/通信/...
        工作目录/输出/<人名>/报告/...
   - 通信“ALL 汇总统计”放在：工作目录/输出/通信汇总/统计/ALL-通信统计-按号码.xlsx
"""
...
@dataclass
class AppState:
    OUT_DIR: Optional[Path] = None  # 输出根：工作目录/输出
    ...
    def in_out_dir(self, p: Path) -> bool:
        if self.OUT_DIR is None:
            return False
        try:
            return p.resolve().is_relative_to(self.OUT_DIR.resolve())  # py>=3.9
...
def out_bank_dir(person: str) -> Path:
    d = out_person_root(person) / "银行"
    d.mkdir(parents=True, exist_ok=True)
    return d


def out_comm_dir(person: str) -> Path:
    d = out_person_root(person) / "通信"
    d.mkdir(parents=True, exist_ok=True)
    return d


def out_report_dir(person: str) -> Path:
    d = out_person_root(person) / "报告"
    d.mkdir(parents=True, exist_ok=True)
    return d
...
def _parse_compact_datetime(s: Any) -> Optional[str]:
    ...
    try:
        dt = _dt.datetime(y, m, d, hh, mm, ss)
...
def should_skip_special(path_str: str) -> Optional[str]:
    ...
    try:
        ...
        for kw in SKIP_HEADER_KEYWORDS:
            if (
                head.astype(str)
                .apply(lambda col: col.astype(str).str.contains(kw, na=False))
                .any()
                .any()
            ):
                return kw
...
def save_df_auto_width(
    ...
    """
    filename 支持相对路径（相对输出根目录 out_root()）或绝对路径。
    """
...
def csv_to_template(raw: pd.DataFrame, holder: str, feedback_unit: str) -> pd.DataFrame:
    ...
    try:
        ...
        def _std_success(v):
            s = safe_str(v).strip()
            if s in {"1", "Y", "y", "是", "成功", "True", "true"}:
                return "成功"
            if s in {"0", "N", "n", "否", "失败", "False", "false"}:
                return "失败"
            return s
        ...
        out["币种"] = (
            col(["交易币种", "币种", "币别", "货币"], "CNY")
            .map(safe_str)
            .replace({"人民币": "CNY", "人民币元": "CNY", "RMB": "CNY", "156": "CNY"})
            .fillna("CNY")
        )
        ...
        out["交易是否成功"] = succ.map(lambda v: "成功" if safe_str(v).strip() in {"1","Y","y","是","成功","True","true"} else ("失败" if safe_str(v).strip() in {"0","N","n","否","失败","False","false"} else safe_str(v)))
        ...
        try:
            ...
            out["备注"] = np.where(
                reason != "",
                np.where(beizhu != "", beizhu + "｜原因：" + reason, "原因：" + reason),
                beizhu,
            )
        except Exception:
            out["备注"] = _S("")
...
def rc_to_template(raw: pd.DataFrame, holder: str, is_old: bool) -> pd.DataFrame:
    ...
    dates = col("交易日期").map(safe_str)
    times = col("交易时间").map(safe_str)
    out["交易时间"] = [_parse_dt(d, t, is_old) for d, t in zip(dates, times)]
    out["借贷标志"] = col("借贷标志").map(safe_str)
    out["币种"] = (
        "CNY"
        if is_old
        else col("币种").map(safe_str).replace("人民币", "CNY").replace("人民币元", "CNY")
    )
    out["查询对象"] = holder
...
def _merge_abc_datetime(date_val, time_val) -> str:
    ...
    date_ts = (
        pd.to_datetime(ds[:8], format="%Y%m%d", errors="coerce")
        if len(ds) >= 8
        else pd.to_datetime(date_val, errors="coerce")
    )
    if pd.isna(date_ts):
        return "wrong"
...
def ccb_offline_from_file(p: Path) -> pd.DataFrame:
    ...
    dates = df.get("交易日期", "")
    times = df.get("交易时间", "")
    times_str = pd.Series(times).map(safe_str).str.replace(r"\.0$", "", regex=True)
    ...
    ext = df.get("扩充备注", "").map(safe_str).replace({"nan": ""})
    out["备注"] = ext
...
def _guess_header_row_strict(xls: pd.ExcelFile, sheet_name: str, scan_rows: int = 30) -> Optional[int]:
    ...
    required = set(STRICT_CONTACTS_REQUIRED)
...
def load_contacts_phone_map_strict(root: Path) -> Dict[str, Tuple[str, str]]:
    ...
    repo_files = [
        p
        for p in root.rglob("*通讯录*.xls*")
        if ("已标注" not in p.stem) and (not STATE.in_out_dir(p))
    ]
    ...
    for p in all_files:
        ...
        for sht in xls.sheet_names:
            try:
                ...
                if hit:
                    for k, row in dtmp.set_index("号码")[["姓名", "职务"]].to_dict("index").items():
                        merged[k] = (row["姓名"], row["职务"])
            except Exception as e:
...
# ========= 通信标注 + 统计 =========
def _find_header_row_exact(
    xls: pd.ExcelFile, sheet_name: str, required_cols: List[str], scan_rows: int = 40
) -> Optional[int]:
    df0 = xls.parse(sheet_name, header=None, nrows=scan_rows)
...
def _enrich_comm_strict(df: pd.DataFrame, phone_map: Dict[str, Tuple[str, str]]) -> pd.DataFrame:
    ...
    if "对方姓名" not in d.columns:
        d["对方姓名"] = ""
    if "对方职务" not in d.columns:
        d["对方职务"] = ""
...
def _stats_by_phone(enriched_df: pd.DataFrame) -> pd.DataFrame:
    ...
    norm_map = {val: normalize_phone_cell(val) for val in uniq}
    d["__对方号码__"] = d["对方号码"].map(norm_map)
    ...
    fest_counts = grp.apply(
        lambda g: pd.Series({f"{fname}通信次数": int((fest_ser.loc[g.index] == fname).sum()) for fname in FESTIVAL_NAMES})
    )
...
def load_and_enrich_communications_strict(root: Path, phone_to_name_title: Dict[str, Tuple[str, str]]) -> Dict[str, str]:
    """
    输出改到（人名文件夹）：
      输出/<人名>/通信/已标注/xxx-已标注.xlsx
      输出/<人名>/通信/统计/xxx-通信统计-按号码.xlsx
    ALL 汇总：
      输出/通信汇总/统计/ALL-通信统计-按号码.xlsx
    """
    ...
    if all_enriched_frames:
        ...
        if STATE.COMM_STATS_ALL is not None and not STATE.COMM_STATS_ALL.empty:
            ...
            print("✅ ALL通信统计汇总导出：输出/通信汇总/统计/ALL-通信统计-按号码.xlsx")
...
def merge_all_txn(root_dir: str) -> pd.DataFrame:
    ...
    if not dup_df.empty:
        ...
        print(f"✅ 已导出重复交易流水：{len(dup_df)} 条 -> 输出/所有人-重复交易流水.xlsx")
    ...
    print("✅ 已导出：输出/所有人-合并交易流水.xlsx")
    return all_txn


# ========= 分析输出（输出/<人名>/银行/...；文件名保持不变） =========
...
def build_person_report_txt(root: Path, person: str, df_person: pd.DataFrame) -> Path:
    ...
    if df_person is None or df_person.empty:
        ...
        d = df_person.copy()
        for c in [
            "借贷标志", "交易金额", "交易时间", "反馈单位", "交易对方姓名", "对方职务",
        ...
        if not bal_valid.empty:
            ...
            last_per_acct = bal_valid.groupby(["反馈单位", "__acct_id__"], dropna=False).tail(1)[
                ["反馈单位", "__acct_id__", "账户余额", "交易时间"]
            ]
            bank_sum = (
                last_per_acct.groupby("反馈单位", dropna=False)["账户余额"]
                .sum()
                .reset_index()
            ...
            lines.append(
                "从余额字段可提取的情况看，按各银行账户末笔余额汇总后，"
                + "；".join(parts)
                + f"。银行余额总额约{_fmt_money_human(total_balance)}。"
            )
        else:
    ...
    if not re_person:
        ...
        lines.append(
            f"经检索汇总，已匹配不动产文件{_fmt_human_num(re_file_cnt)}个，提取有效记录{_fmt_human_num(len(re_person))}条。"
            "现按字段摘述房屋坐落、建筑面积、交易价格（万元）、登记时间等要素如下。"
        )
        take = re_person[:3]
        sents = []
        for r in take:
...
def create_gui():
    ...
    tip = (
        f"tips1：输出根目录固定为“{OUTPUT_ROOT_NAME}”（即 工作目录/{OUTPUT_ROOT_NAME}）。\n"
        "tips2：每个人单独一个文件夹（你说的“xxx”=人名）：输出/<人名>/银行、通信、报告。\n"
        "tips3：汇总文件（所有人-合并交易流水.xlsx、所有人-重复交易流水.xlsx）仍在 输出 根目录（保持不变）。\n"
        "tips4：通讯录文件需表头包含：姓名、职务、号码；通话记录表头至少包含：对方号码。\n"
        "tips5：不动产信息需文件名符合“人名-全省不动产.xlsx”，报告只体现：房屋坐落、建筑面积、交易价格（万元）、登记时间。"
    )

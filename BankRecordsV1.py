...
import sys, re, threading, warnings, builtins
...
def out_person_root(person: str) -> Path:
    p = out_root() / _sanitize_person_name(person)
    p.mkdir(parents=True, exist_ok=True)
    return p

def out_bank_dir(person: str) -> Path:
    d = out_person_root(person) / "银行"; d.mkdir(parents=True, exist_ok=True); return d
def out_comm_dir(person: str) -> Path:
    d = out_person_root(person) / "通信"; d.mkdir(parents=True, exist_ok=True); return d
def out_report_dir(person: str) -> Path:
    d = out_person_root(person) / "报告"; d.mkdir(parents=True, exist_ok=True); return d
def out_gongan_dir(person: str) -> Path:
    d = out_person_root(person) / "公安信息"; d.mkdir(parents=True, exist_ok=True); return d
def out_comm_summary_dir() -> Path:
    d = out_root() / "通信汇总"; d.mkdir(parents=True, exist_ok=True); return d
...
def _parse_dt(d: Any, t: Any, is_old: bool) -> str:
    try:
        s_d, s_t = safe_str(d).strip(), safe_str(t).strip()
        r = _parse_compact_datetime(s_d) or _parse_compact_datetime(s_t)
        if r:
            return r
        dd, tt = ONLY_DIGITS_RE.sub("", s_d), ONLY_DIGITS_RE.sub("", s_t)
        if COMPACT_DT_DIGITS_RE.fullmatch(dd) or COMPACT_DT_DIGITS_RE.fullmatch(tt):
            r = _parse_compact_datetime(dd) or _parse_compact_datetime(tt)
            if r:
                return r
        if len(dd) >= 8 and len(tt) >= 6:
            r = _parse_compact_datetime(dd[:8] + tt[:6])
            if r:
                return r
...
def should_skip_special(path_str: str) -> Optional[str]:
    p = Path(path_str)
    try:
        head = pd.read_excel(p, header=None, nrows=3)
        m = head.astype(str).apply(lambda col: col.astype(str).str.contains("|".join(map(re.escape, SKIP_HEADER_KEYWORDS)), na=False)).any().any()
        if not m:
            return None
...
def _read_csv_smart(p: Path, **kwargs) -> pd.DataFrame:
    for enc in ["utf-8-sig", "gb18030", "utf-8", "cp936"]:
        try:
            return pd.read_csv(p, encoding=enc, **kwargs)
        except Exception:
            pass
    raise RuntimeError(f"无法读取CSV: {p}")
...
def extract_holder_from_df(raw: pd.DataFrame) -> str:
    for col in raw.columns:
        if any(k in str(col) for k in NAME_CANDIDATE_COLS):
            s = raw[col].dropna()
            if not s.empty:
...
def csv_to_template(raw: pd.DataFrame, holder: str, feedback_unit: str) -> pd.DataFrame:
    ...
    out["查询账户"], out["查询卡号"] = out["本方账号"], out["本方卡号"]

    opp_no = col(["交易对手账卡号","交易对手账号","对方账号","对方账户"]).map(_to_str_no_sci)
    opp_typ = col(["交易对方帐卡号类型","账号/卡号类型"], "").map(safe_str)
    is_card = opp_typ.str.contains("卡", na=False) | opp_typ.isin(["2","卡","卡号"])
    ...
    out["币种"] = col(["交易币种","币种","币别","货币"], "CNY").map(safe_str).replace(
        {"人民币":"CNY","人民币元":"CNY","RMB":"CNY","156":"CNY"}
    ).fillna("CNY")
    ...
    out["交易对方姓名"] = col(
        ["对手户名","交易对手名称","对手方名称","对方户名","对方名称","对方姓名","收/付方名称"], " "
    ).map(safe_str)
    ...
    out["日志号"] = col(["日志号"], "").map(safe_str)
    out["传票号"] = col(["传票号"], "").map(safe_str)
    out["凭证种类"] = col(["凭证种类","凭证类型"], "").map(safe_str)
    out["凭证号"] = col(["凭证号","凭证序号"], "").map(safe_str)
    out["现金标志"] = col(["现金标志"], "").map(safe_str)
    out["终端号"] = col(["终端号","渠道号"], "").map(safe_str)

    succ = col(["交易是否成功","查询反馈结果"], "").map(safe_str).str.strip()
    out["交易是否成功"] = succ.map(lambda v:
        "成功" if v in {"1","Y","y","是","成功","True","true"} else
        ("失败" if v in {"0","N","n","否","失败","False","false"} else v)
    )

    out["交易发生地"] = col(["交易发生地","交易场所"], "").map(safe_str)
    out["商户名称"] = col(["商户名称"], "").map(safe_str)
    out["商户号"] = col(["商户号"], "").map(safe_str)
    out["IP地址"] = col(["IP地址"], "").map(safe_str)
    out["MAC"] = col(["MAC地址","MAC"], "").map(safe_str)
    out["交易柜员号"] = col(["交易柜员号","柜员号","记账柜员"], "").map(safe_str)

    beizhu = col(["备注","附言","说明"], "").map(safe_str)
    reason = col(["查询反馈结果原因"], "").map(safe_str)
...
def tl_to_template(raw) -> pd.DataFrame:
    if isinstance(raw, dict):
        ...
        for sheet, d in raw.items():
            one = tl_to_template(d)
            if not one.empty:
                one.insert(0, "__sheet__", sheet)
                frames.append(one)
    ...
    out["币种"] = col_multi(["币种","货币","币别"]).replace({"156":"CNY","人民币元":"CNY","人民币":"CNY"}).fillna("CNY")
    ...
    out["凭证种类"] = col_multi(["凭证类型"], "").map(safe_str)
    out["凭证号"] = col_multi(["凭证序号"], "").map(safe_str)
    return out.reindex(columns=TEMPLATE_COLS, fill_value="")
...
def mt_to_template(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    df.columns = raw.iloc[header_idx].map(safe_str).str.strip()
    df.dropna(how="all", inplace=True); df.reset_index(drop=True, inplace=True)

    summary_mask = df.apply(lambda r: r.map(safe_str).str.contains(r"支出笔数|收入笔数|支出累计金额|收入累计金额").any(), axis=1)
    ...
    out["本方账号"] = acct; out["查询账户"] = acct
    out["查询对象"] = holder; out["反馈单位"] = "民泰银行"
    out["币种"] = col("币种").map(safe_str).replace({"人民币":"CNY","人民币元":"CNY"}).fillna("CNY")
    ...
    def _fmt_time(v: Any) -> str:
        ...
        try:
            ...
            pass
        try:
            return _dt.datetime.strptime(s, "%Y%m%d %H:%M:%S").strftime("%Y-%m-%d %H:%M:%S")
        except Exception:
            return s or "wrong"

    out["交易时间"] = col("时间").map(_fmt_time)
    out["交易摘要"] = col("摘要", " ").map(safe_str)
    out["交易流水号"] = col("柜员流水号").map(safe_str).str.strip()
    out["交易柜员号"] = col("记账柜员 ").map(safe_str).str.strip()
    out["交易网点代码"] = col("记账机构").map(safe_str).str.strip()
    ...
    out["终端号"] = col("交易渠道").map(safe_str)
    out["备注"] = col("附言", " ").map(safe_str)
    return out.reindex(columns=TEMPLATE_COLS, fill_value="")
...
def rc_to_template(raw: pd.DataFrame, holder: str, is_old: bool) -> pd.DataFrame:
    ...
    out["本方账号"] = col("账号", "wrong").map(safe_str)
    out["查询账户"] = out["本方账号"]
    ...
    out["币种"] = "CNY" if is_old else col("币种").map(safe_str).replace({"人民币":"CNY","人民币元":"CNY"})
    out["查询对象"] = holder
    out["交易对方姓名"] = col("对方姓名", " ").map(safe_str)
    out["交易对方账户"] = col("对方账号", " ").map(safe_str)
...
def _merge_abc_datetime(date_val, time_val) -> str:
    s_date, s_time = safe_str(date_val).strip(), safe_str(time_val).strip()
    r = _parse_compact_datetime(s_date) or _parse_compact_datetime(s_time)
    if r:
        return r
    ...
    def to_hhmmss(t) -> str:
        if t is None or (isinstance(t, float) and np.isnan(t)) or t == "" or pd.isna(t):
            return "000000"
        if isinstance(t, (int, np.integer, float, np.floating)):
            try:
                ...
                if 0 <= tf < 1:
                    ...
                    secs = 0 if secs >= 86400 else secs
        ...
        s = safe_str(t).strip()
        if ":" in s or "." in s:
            tt = pd.to_datetime("2000-01-01 " + s, errors="coerce")
            if pd.notna(tt):
...
def abc_offline_from_file(p: Path) -> pd.DataFrame:
    ...
    dates, times = df.get("交易日期", ""), df.get("交易时间", "")
    out["交易时间"] = [_merge_abc_datetime(d, t) for d, t in zip(dates, times)]

    out["交易摘要"] = df.get("摘要", "").map(safe_str)
    ...
    for c in ["交易流水号","交易类型","交易对方卡号","交易对手余额","日志号","凭证种类","凭证号","现金标志",
              "交易是否成功","交易发生地","商户名称","商户号","IP地址","MAC","交易柜员号","备注"]:
        out[c] = ""
    return out.reindex(columns=TEMPLATE_COLS, fill_value="")
...
def ccb_offline_from_file(p: Path) -> pd.DataFrame:
    ...
    out["借贷标志"] = np.where(
        jd.str.contains("^贷", na=False) | jd.str.upper().isin(["贷","C","CR","CREDIT"]),
        "进",
        np.where(
            jd.str.contains("^借", na=False) | jd.str.upper().isin(["借","D","DR","DEBIT"]),
            "出",
    ...
    dates = df.get("交易日期", "")
    times = pd.Series(df.get("交易时间", "")).map(safe_str).str.replace(r"\.0$", "", regex=True)
    out["交易时间"] = [_parse_dt(d, t, False) for d, t in zip(dates, times)]
    ...
    out["备注"] = df.get("扩充备注", "").map(safe_str).replace({"nan": ""})

    for c in ["查询项","交易类型","交易对方卡号","交易对手余额","日志号","传票号","凭证种类","凭证号",
              "现金标志","交易是否成功","交易发生地"]:
        out[c] = ""
    return out.reindex(columns=TEMPLATE_COLS, fill_value="")
...
def _stats_by_phone(enriched_df: pd.DataFrame) -> pd.DataFrame:
    ...
    offwork = _flag_offwork(ts)
    late = _flag_late_night(ts)
    ge3 = d["__dur_sec__"] >= 180
    fest = _festival_series(ts)

    # 统计主表（尽量少 apply）
    grp = d.groupby("__对方号码__", dropna=False)
    base = grp.size().rename("通信次数").to_frame()
    base["非工作时间通信次数"] = grp.apply(lambda g: int(offwork.loc[g.index].sum())).values
    base["深夜通信次数(23–5)"] = grp.apply(lambda g: int(late.loc[g.index].sum())).values
    base["通话≥3分钟次数"] = grp.apply(lambda g: int(ge3.loc[g.index].sum())).values
    base["姓名"] = grp.apply(lambda g: _mode_nonempty(nm.loc[g.index])).values
    base["职务"] = grp.apply(lambda g: _mode_nonempty(title.loc[g.index])).values
...
def load_and_enrich_communications_strict(root: Path, phone_to_name_title: Dict[str, Tuple[str, str]]) -> Dict[str, str]:
    ...
    if not files:
        print(f"ℹ️ 未发现符合命名“（人名）-{COMM_SUFFIX}.xlsx/.xls”的通信文件。")
        return {}
    ...
    for p in files:
        person_guess = safe_str(_person_from_comm_filename(p)).strip() or "未知"
        ...
        for sht in xls.sheet_names:
            ...
            STATE.COMM_RECORDS_COUNT += len(enriched)
            STATE.COMM_RECORDS_BY_PERSON[person_guess] = STATE.COMM_RECORDS_BY_PERSON.get(person_guess, 0) + len(enriched)
        ...
        if frames:
            ...
            comm_mark_dir = out_comm_dir(person_guess) / "已标注"
            comm_stat_dir = out_comm_dir(person_guess) / "统计"
            comm_mark_dir.mkdir(parents=True, exist_ok=True)
            comm_stat_dir.mkdir(parents=True, exist_ok=True)

            save_df_auto_width(merged, comm_mark_dir / f"{p.stem}-已标注", index=False, engine="openpyxl")
            print(f"✅ 通信标注导出：{comm_mark_dir / (p.stem + '-已标注.xlsx')}")

            all_enriched_frames.append(merged)

            stat_df = _stats_by_phone(merged)
            if stat_df is not None and not stat_df.empty:
                save_df_auto_width(stat_df, comm_stat_dir / f"{p.stem}-通信统计-按号码", index=False, engine="openpyxl")
                print(f"✅ 通信统计导出：{comm_stat_dir / (p.stem + '-通信统计-按号码.xlsx')}")
    ...
    if all_enriched_frames:
        ...
        comm_sum_stat_dir = out_comm_summary_dir() / "统计"
        comm_sum_stat_dir.mkdir(parents=True, exist_ok=True)
        ...
        if "查询对象" in merged_all.columns and "对方号码" in merged_all.columns:
            ...
            if common_data:
                ...
                # 由于不同联系人关联的核心对象不同，未关联的对象列会是 NaN，需要填 0 并转为整数
...
def merge_all_txn(root_dir: str) -> pd.DataFrame:
    ...
    if comm_files:
        print(f"📁 检测到 {len(comm_files)} 个“（人名）-{COMM_SUFFIX}”通信文件，将加载通讯录并进行通信标注。")
        ...
        print(f"ℹ️ 未发现符合命名“（人名）-{COMM_SUFFIX}.xlsx/.xls”的通信文件，本次不读取通讯录，也不做通信标注；同时不做“同住/同机”职务匹配。")
    ...
    print(
        ...
        f"建行线下 {len(ccb_offline_files)}，CSV {len(csv_txn_files)}；通信映射 {len(STATE.CALLLOG_NAME_TO_TITLE)} 条。"
    )
    ...
    for p in tl_files:
        ...
        for sht in xls.sheet_names:
            try:
                xls_dict[sht] = xls.parse(sheet_name=sht, header=header_idx)
            except Exception as e:
    ...
    raw_txn["交易金额"] = pd.to_numeric(raw_txn.get("交易金额", 0), errors="coerce").round(2)

    dup_mask = raw_txn.duplicated(subset=["交易流水号","交易时间","交易金额"], keep="first")
    dup_df = raw_txn[dup_mask].copy()
    STATE.DUP_TXN_COUNT = int(dup_mask.sum())
    if not dup_df.empty:
        save_df_auto_width(dup_df, "所有人-重复交易流水", index=False, engine="openpyxl")
        print(f"✅ 已导出重复交易流水：{len(dup_df)} 条 -> 分析结果/所有人-重复交易流水.xlsx")

    before = len(raw_txn)
    all_txn = raw_txn.drop_duplicates(keep="first").reset_index(drop=True)
    ...
    all_txn["借贷标志"] = all_txn["借贷标志"].apply(lambda x:
        "出" if safe_str(x).strip() in {"1","借","D"} else ("进" if safe_str(x).strip() in {"2","贷","C"} else safe_str(x))
    )
    ...
    all_txn["对方职务"] = all_txn["交易对方姓名"].map(STATE.CALLLOG_NAME_TO_TITLE).fillna("")
...
def analysis_txn(df: pd.DataFrame) -> None:
    ...
    cash = df[
        (df["现金标志"].map(safe_str).str.contains("现", na=False) |
...
def _build_top8_partner_narratives(d: pd.DataFrame, person: str, top_n: int = 8) -> List[str]:
    ...
    for i, name in enumerate(top_names, start=1):
        ...
        in_sum = float(g.loc[g["__is_in__"], "__abs_amt__"].sum(skipna=True) or 0.0)
        out_sum = float(g.loc[g["__is_out__"], "__abs_amt__"].sum(skipna=True) or 0.0)
...
def build_person_report_txt(root: Path, person: str, df_person: pd.DataFrame) -> Path:
    ...
    if df_person is None or df_person.empty:
        ...
        need_cols = ["借贷标志","交易金额","交易时间","反馈单位","交易对方姓名","对方职务","账户余额","现金标志","交易类型","查询账户","查询卡号",
                     "交易对方账号开户行","交易对方账户","交易对方卡号","交易摘要","备注"]
        for c in need_cols:
        ...
        if not bal_valid.empty:
            ...
            parts = [f"{safe_str(r['反馈单位']).strip() or '（未知银行）'}合计约{_fmt_money_human(float(r['账户余额'] or 0))}" for _, r in bank_sum.iterrows()]
    ...
    rec_cnt = int(STATE.COMM_RECORDS_BY_PERSON.get(person, 0))

    if st is None or st.empty:
        lines += [f"经检索，本次未能获取“{person}”对应的已生成通信统计表（请确认是否存在“{person}-{COMM_SUFFIX}.xlsx/.xls”）。", ""]
    else:
        lines.append(
        ...
        for _, r in topc.iterrows():
            ...
            c1 = int(pd.to_numeric(r.get("通信次数",0), errors="coerce") or 0)
            c2 = int(pd.to_numeric(r.get("非工作时间通信次数",0), errors="coerce") or 0)
            c3 = int(pd.to_numeric(r.get("深夜通信次数(23–5)",0), errors="coerce") or 0)
            c4 = int(pd.to_numeric(r.get("通话≥3分钟次数",0), errors="coerce") or 0)
...
def create_gui():
    ...
    tip = (
        ...
        f"tips2：通信文件命名需严格为“（人名）-{COMM_SUFFIX}.xlsx（或 .xls）”，表头至少包含：对方号码。\n"
    ...
    def run(path: str):
        ...
        STATE.OUT_DIR = Path(path).expanduser().resolve() / OUTPUT_ROOT_NAME
